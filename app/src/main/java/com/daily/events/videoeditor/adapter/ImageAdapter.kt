...
class ImageAdapter(
    ...
    override fun onBindViewHolder(viewHolder: RecyclerView.ViewHolder, position: Int) {
        ...
        if (Constant.selectedImages.size == 0) {
            holder.imageViewBinding.mImageTrans.visibility = View.GONE
        } else {
            for (i in 0 until Constant.selectedImages.size) {
                val path1: String = Constant.selectedImages[i].getPath()!!
                if (file.getPath().equals(path1)) {
                    holder.imageViewBinding.mImageTrans.visibility = View.VISIBLE
                    break
                } else {
                    holder.imageViewBinding.mImageTrans.visibility = View.GONE
                }
